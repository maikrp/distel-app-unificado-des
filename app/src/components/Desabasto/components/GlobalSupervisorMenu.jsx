...
export default function GlobalSupervisorMenu({ usuario }) {
  ...
  /* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
  const getBarColor = (p) => {
  const valor = Math.floor(p * 100) / 100; // fuerza a dos decimales exactos

    if (valor === 100) return "bg-green-700";     // verde oscuro 100 %
    if (valor >= 99 && valor < 100) return "bg-green-600";  // verde medio 99.x
    if (valor >= 95 && valor < 99) return "bg-yellow-400";  // amarillo 95–98.x
    if (valor >= 80 && valor < 95) return "bg-orange-500";  // naranja
  ...
  const getPctTextColor = (p) => {
    ...
    if (p >= 98) return "text-green-500";
    if (p >= 90) return "text-yellow-500";
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex justify-center px-4 py-6 sm:p-6">
        <div className="bg-white shadow-lg rounded-3xl p-6 w-full max-w-5xl">
          ...
          <div className="bg-gray-300 rounded-full h-4 overflow-hidden w-full max-w-md mb-2">
            <div
              className={`${barZona} h-4 transition-all duration-500`}

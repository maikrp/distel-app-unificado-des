...
export default function AdminToolsPanel({ onVolver }) {
  ...
  const cr = new Date().toLocaleString("sv-SE", { timeZone: "America/Costa_Rica" });
    return new Date(cr);
  };
  ...
  const manejarCarga = async () => {
    try {
      ...
      const jsonNorm = jsonOriginal.map((row, idx, arr) => {
        ...
        for (const k of Object.keys(row)) {
          ...
          if (typeof val === "string") {
            ...
            // Normalización avanzada de valores vacíos o no válidos
            if (
              t === "" ||
              t === "-" ||
              t.toLowerCase() === "nan" ||
              ...
              // Convertir texto numérico a número real (ej. "123.45" → 123.45)
              nuevo[destino] = Number(t);
            } else {
              nuevo[destino] = t;
            }
          } else if (typeof val === "number" && !isFinite(val)) {
            // Manejo de valores numéricos infinitos o NaN
            nuevo[destino] = null;
          } else {
            nuevo[destino] = val;
          }
      ...
      const fechaCR = new Date().toLocaleString("sv-SE", { timeZone: "America/Costa_Rica" });

      // === Validación de columnas ===
      const colsExcel = Object.keys(filtrados[0] || {});
      const desconocidas = colsExcel.filter((c) => !allowedColumns.includes(c));

      if (desconocidas.length > 0) {
        console.warn("Columnas desconocidas detectadas:", desconocidas);
        notify(
          `⚠️ Se detectaron ${desconocidas.length} columnas no reconocidas:\n${desconocidas.join(
            ", "
          )}\n\nSerán ignoradas al insertar.`
        );
      }
      const procesados = filtrados.map((r) => {
        ...
        // Asegurar columna agente_id esperada por triggers Supabase
        if (!("agente_id" in soloPermitidos)) {
          soloPermitidos.agente_id = null;
        }
        return soloPermitidos;
      });

      // === Filtrado final de registros completamente vacíos ===
      const procesadosFiltrados = procesados.filter((fila) => {
        const valores = Object.values(fila);
        // true si hay al menos un campo no nulo ni vacío
        return valores.some((v) => v !== null && v !== "" && v !== undefined);
      });

      if (procesadosFiltrados.length === 0) {
        setProgressDetalle("Todas las filas estaban vacías o sin datos útiles.");
        setProgressPct(100);
        setProgressTerminado(true);
        notify("⚠️ No se encontraron filas válidas con información para insertar.");
        return;
      }

      const total = procesadosFiltrados.length;
      if (total === 0) {
      ...
      const lote = 200; // tamaño de lote reducido para estabilidad Supabase
      let insertados = 0;
      for (let i = 0; i < total; i += lote) {
        const subset = procesadosFiltrados.slice(i, i + lote);
        if (subset.length === 0) continue;
        const fechaCR = new Date().toLocaleString("sv-SE", { timeZone: "America/Costa_Rica" });

        // === Validación y reintento automático por lote ===
        let intento = 0;
        let exito = false;
        let error = null;
        ...
        while (intento < 3 && !exito) {
          try {
            const resultado = await supabase.from("desabasto_registros").insert(subset);
            error = resultado.error || null;
            if (!error) {
              console.log(`✅ Lote ${i / lote + 1} insertado correctamente (${subset.length} registros).`);
              exito = true;
              break;
            } else {
              intento++;
              console.warn(
                `⚠️ Error en lote ${i / lote + 1}, intento ${intento}:`,
                error.message
              );
              await new Promise((r) => setTimeout(r, 1000 * intento)); // pausa incremental
            }
          } catch (e) {
            intento++;
            error = e;
            console.error(
              `❌ Excepción en lote ${i / lote + 1}, intento ${intento}:`,
              e.message
            );
        ...
        if (error && !exito) {
          console.error(`⛔ Lote ${i / lote + 1} falló tras 3 intentos:`, error.message);
          notify(`❌ Error al insertar lote ${(i / lote) + 1}: ${error.message}`);
          continue; // pasar al siguiente lote sin detener toda la carga
        }
      ...
      // === Registro de resumen y auditoría de carga ===
      try {
        const finCR = new Date().toLocaleString("sv-SE", { timeZone: "America/Costa_Rica" });
        const resumen = {
          fecha_inicio: fechaCR,
          fecha_fin: finCR,
          total_registros_excel: jsonNorm.length,
          total_filtrados: filtrados.length,
          total_enviados: insertados,
          total_excluidos: excluidos,
          ejecutor: localStorage.getItem("usuario") || "Desconocido",
          fuente_archivo: archivo.name,
        };

        const { error: auditError } = await supabase.from("control_registro").insert(resumen);
        if (auditError) {
          console.warn("⚠️ No se pudo registrar auditoría de carga:", auditError.message);
        } else {
          console.log("🧾 Auditoría de carga registrada correctamente en control_registro.");
        }
      } catch (errAudit) {
        console.error("❌ Error registrando auditoría de carga:", errAudit.message);
      }
